...
const TokenPage = () => {
  ...
  }, [symbol, tokenTransactions]);

  const formatTokenPrice = useCallback((price, isStablecoin) => {
  ...
  return (
    ...
    >
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className={`token-page__change ${isPositive ? 'up' : 'down'}`}>
                    {isPositive ? <ChangeUp width={16} height={16}/> :
                      <ChangeDown width={16} height={16}/>}
      ...
      {!loading && tokenData?.token && (
        <FooterPanelToken
          tokenSymbol={tokenData.token.symbol}
          tokenDbSymbol={tokenData.token.db_symbol || tokenData.token.symbol.toLowerCase()}
          tokenData={tokenData.token}
