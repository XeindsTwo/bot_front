...
const SendPage = () => {
  ...
  const displaySymbol = tokenData?.symbol || symbol.toUpperCase().split('_')[0];
  ...
  const handleBlur = (field) => {
    ...
    if (field === 'address') setErrors({...errors, address: validateAddress(formData.address)});
    else if (field === 'amount') setErrors({...errors, amount: validateAmount(formData.amount)});
  };
  ...
  if (!tokenData) return <PageHeader title="Loading..." backUrl={-1}/>;

  return (
      <>
        <PageHeader title={`Send ${displaySymbol}`} backUrl={-1}/>
        <TokenHeader
